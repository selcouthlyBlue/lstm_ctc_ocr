...
def accuracy_calculation(original_seq, decoded_seq, ignore_value=0, is_print=True):
    ...
    for i, origin_label in enumerate(original_seq):
        ...
        if is_print and i < cfg.VAL.PRINT_NUM:
            print('seq{0:4d}: origin: {1} decoded:{2}'.format(i, origin_label, decoded_label))
        if org_label == decoded_label:
            count += 1
