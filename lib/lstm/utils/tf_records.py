...
def sparse_tuple_from_label(sequences, dtype=np.int32):
    ...
    indices.extend(zip(sequences * len(sequences), range(len(sequences))))
...
def write_image_annotation_pairs_to_tfrecord(labels_file, tfrecords_filename):
    ...
    data_dir = 'C:/Users/asus.11/Documents/Deep Learning/ocr playground/data/'
    ...
    for item in labeled_data:
        if i % 1000 == 0:
            print("Data: ", i, "/", len(labeled_data))
        split_item = item.split()
        if split_item[1] == 'ok':
            image_name = data_dir + "/" + split_item[0] + ".png"
            img = np.array(Image.open(image_name))
            code = split_item[-1].replace("|", " ")
            code = [cfg.BLANK_INDEX if code == cfg.BLANK_TOKEN else encode_maps[c] for c in list(code)]
            aligned_code = code[:]
            while len(aligned_code) < maxLen:
                aligned_code.append(0)
...
def read_tfrecord_and_decode_into_image_annotation_pair_tensors(tfrecord_filenames_queue):
    ...
    with tf.Session().as_default():
        label = label.eval()
    label = sparse_tuple_from_label(label)
...
def wrtie_test(labels_file, tfrecords_filename=None):
    write_image_annotation_pairs_to_tfrecord(labels_file=labels_file, tfrecords_filename=tfrecords_filename)
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            while not coord.should_stop():
                # get one batch
                img_batch, l_batch, l_len_batch, t_s_batch = sess.run(
                    [image_batch, label_batch, label_len_batch, time_step_batch])
                label = []
                for l in l_batch:
                    while l[-1] == 0:
                        l = l[:-1]
...
if __name__ == '__main__':
    wrtie_test(labels_file='C:/Users/asus.11/Documents/Deep Learning/ocr playground/lines.txt',
               tfrecords_filename='../data/train_4_6.tfrecords')
