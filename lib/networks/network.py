...
class Network(object):
    ...
    @layer
    def conv_single(self, input, k_h, k_w, c_o, s_h, s_w, name, c_i=None, biased=True, relu=True,
        ...
        if c_i == 1:
            input = tf.expand_dims(input=input, axis=3)
    ...
    @layer
    def conv(self, input, k_h, k_w, c_o, s_h, s_w, name, c_i=None, biased=True, relu=True, padding=DEFAULT_PADDING,
             trainable=True):
        """ contribution by miraclebiu, and biased option"""
        self.validate_padding(padding)
        if not c_i:
    ...
    @layer
    def conv_norm(self, input, k_h, k_w, c_o, s_h, s_w, name, biased=True, relu=True, padding=DEFAULT_PADDING,
                  trainable=True):
        """ contribution by miraclebiu, and biased option"""
        self.validate_padding(padding)
        c_i = input.get_shape()[-1]
    ...
    @layer
    def spatial_reshape_layer(self, input, d):
        input_shape = tf.shape(input)
    ...
    @layer
    def bn_scale_combo(self, input, name, relu=True):
    ...
    @layer
    def pva_negation_block(self, input, k_h, k_w, c_o, s_h, s_w, name, biased=True, padding=DEFAULT_PADDING,
                           trainable=True,
                           scale=True, negation=True):
        """ for PVA net, Conv -> BN -> Neg -> Concat -> Scale -> Relu"""
        with tf.variable_scope(name):
            conv = self.conv._original(self, input, k_h, k_w, c_o, s_h, s_w, biased=biased, relu=False, name='conv',
    ...
    @layer
    def pva_negation_block_v2(self, input, k_h, k_w, c_o, s_h, s_w, c_in, name, biased=True, padding=DEFAULT_PADDING,
                              trainable=True,
                              negation=True):
        ...
        with tf.variable_scope(name):
            ...
            conv = self.conv._original(self, bn, k_h, k_w, c_o, s_h, s_w, biased=biased, relu=False, name='conv',
                                       padding=padding,
                                       trainable=trainable)
            return conv
    ...
    def pva_inception_res_stack(self, input, c_in, name, block_start=False, type='a'):
        ...
        if block_start:
            ...
            stride = 1
        with tf.variable_scope(name + '/incep'):
            bn = self.batch_normalization._original(self, input, name='bn', relu=False, is_training=False)
            bn_scale = self.scale._original(self, bn, c_in, name='bn_scale')
            # 1 x 1

            conv = self.conv._original(self, bn_scale, 1, 1, c_0, stride, stride, name='0/conv', biased=False,
                                       relu=False)
            ...
            if name == 'conv4_1':
                tmp_c = c_1
                c_1 = 48
            conv = self.conv._original(self, bn_relu, 1, 1, c_1, stride, stride, name='1_reduce/conv', biased=False,
                                       relu=False)
            ...
            # 5 x 5
            conv = self.conv._original(self, bn_scale, 1, 1, c_2, stride, stride, name='2_reduce/conv', biased=False,
                                       relu=False)
            ...
            if block_start:
                ...
                pool = self.conv._original(self, pool, 1, 1, c_pool, 1, 1, name='poolproj/conv', biased=False,
                                           relu=False)
    ...
    @layer
    def pva_inception_res_block(self, input, name_prefix='conv4_', type='a'):
