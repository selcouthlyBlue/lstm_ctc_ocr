...
from lib.lstm.config import get_output_dir, cfg_from_file


class Convert(object):
    def __init__(self, network, model_dir, out_path, model):
        self.net = network
...
def convert_ckpt2npy(network, model_dir, out_path, model):
    ...
    with tf.Session(config=config) as sess:
        ct = Convert(network, model_dir, out_path, model)
        ct.conver2npy(sess)
